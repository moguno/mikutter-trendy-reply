...
Plugin.create(:"mikutter-trendy-reply.rb") {
  # あ〜？もしかしてプラグインだと勘違いさせちゃった？
  # 私ただのモンキーパッチだから。
}

class Gdk::ReplyViewer
  attr_reader :icon_width
  ...
  def render(context)
    if @messages and not @messages.empty?
      @messages.inject(0) { |base_y, message|
        render_single_message(message, context, base_y)
      }
    end
  end
